//
// Created by jeethender on 15-06-2020.
//
...
class Compressor
{
    ...
    Mat dwtImage;//Image DwT quantized
    int height, heightDwT;
    int width, widthDwT;
...
public:
    Compressor();
    virtual ~Compressor(){};
    void setParamCompression(String imageName);
    void setParamDecompression(String imageName);
    void forwardDwT();	// Forward transformation - DwT
    void inverseDwT();	// Inverse transformation - iDwT
    //void displayImage(string imageName, Mat image);// Display all images
    void compressing(String imageName);//Execute Huffman to create a table of symbols and codify the image to a file
    void decompressing(String imageName);
...
void Compressor::setParamCompression(String imageName){
    /// Load image in gray scale
    grayImage = imread( imageName, IMREAD_GRAYSCALE );
    //cvtColor(imageName,grayImage,COLOR_RGB2GRAY);
    ...
    // Necessary to broke the image in 8 by 8 blocks in DwT step
    height = grayImage.size().height;
    width = grayImage.size().width;

    heightDwT = (height%8) ? (heightDwT = 8 - height%8) : 0;
    widthDwT = (width%8) ?  (widthDwT = 8 - width%8) : 0;

    if (heightDwT != 0){
        Mat hBlock = grayImage(Rect(0, height-heightDwT, width, heightDwT));
        vconcat(grayImage, hBlock, grayImage);
        height += heightDwT;
    }

    if (widthDwT != 0){
        Mat wBlock = grayImage(Rect(width-widthDwT, 0, widthDwT, height));
    ...
    outfile << heightDwT << " " << widthDwT << endl;

}


// Forward transformation - DwT
void Compressor::forwardDwT(){

    dwtImage = Mat_<uchar>(height*C/8, width*C/8);
    ...
    for (int i = 0; i < height; i += 8){
        for (int j = 0; j < width; j += 8){
            ...
            // Copying the block back to the new dwt image
            //block8.copyTo(dwtImage(Rect(j, i, 8, 8)));
            Mat blockC = Mat_<uchar>(C, C);
            for (int k = 0; k < (int)C*C; k++)
                blockC.at<uchar>(k) = block8.at<uchar>(zigZagMask[k]);
            //cout << "Copyto\n";
            blockC.copyTo(dwtImage(Rect(j*C/8, i*C/8, C, C)));


        }
    }
    dwtImage.convertTo(dwtImage, CV_8UC1);
}



void Compressor::inverseDwT(){
    grayImage = Mat_<uchar>(height, width);

    for (int i = 0; i < height*C/8; i+=C){
        for (int j = 0; j < width*C/8; j+=C){
            // Get a block 8x8 for each position on image
            Mat block = dwtImage(Rect(j, i, C, C));
...
// Compute the histogram matrix for dwt image in planes
void Compressor::searchFrequenceTable(){
    ...
    Mat histogram;//Histogram for DwT Image

    calcHist( &dwtImage, 1, 0, Mat(), histogram, 1, &histSize, &histRange, uniform, accumulate);
...
//Compressor step
void Compressor::compressing(String imageName){
    ...
    inputFile.assign(dwtImage.datastart, dwtImage.dataend);

    //Codify image dwt based on table of code from Huffman
...
void Compressor::setParamDecompression(String imageName){
    ...
    infile >> size;
    heightDwT = atoi(size.c_str());
    infile >> size;
    widthDwT = atoi(size.c_str());
}
...
//Uncompress step
void Compressor::decompressing(String fileName){
    //Just for information
    //cout << "\nUncompressing...";
    ...
    vector<int> dwtFile = h.decode(codifiedImage);

    dwtImage = Mat::zeros(height*C/8, width*C/8, CV_8UC1);

    //cout << "\nTransforming Image...";
    for (int i = 0; i < height*C/8; i++){
        for (int j = 0; j < width*C/8; j++){
            dwtImage.at<uchar>(i, j) = (uchar)dwtFile.at(i*dwtImage.step + j);
        }
    }
    //cout << "\nProcessing Image...";
    inverseDwT();
    ...
    if (widthDwT != 0)
        grayImage = grayImage.colRange(0, width - widthDwT);

    if (heightDwT != 0)
        grayImage = grayImage.rowRange(0, height - heightDwT);
